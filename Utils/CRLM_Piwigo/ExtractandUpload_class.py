# -*- coding: utf-8 -*-
"""
Created on Wed Mar 02 09:42:33 2016
...
class HisImage():
    ...
    def Login(self):
        return self.mysite.login(username="qmul", password="xuzhaoyang")
    
    def CheckMysite(self):
        if self.webfdlist.has_key(r'/Test1/'+self.finame):
            return True
        else:
            self.mysite.mkdir(r'/Test1/'+self.finame)
            return False
    ...
    def Upload(self,imgname):
        fdpath = r'/Test1/'+self.finame+r'/'+imgname[-1]
    ...
    def NameAnnotation(self,index):
        ...
        elif self.root[index][0].text == 'roi':
    ...
    def ExtractImage(self,index):
        ...
        yi = pl_arr[:,1].min()
        ya = pl_arr[:,1].max()
        tmpim = self.im.read_region((xi-100,yi-100),0,(xa-xi+200,ya-yi+200))
        fig,ax = plt.subplots()
        ax.imshow(tmpim)    
        ax.axis('image')
        ax.axis('off')
        ax.plot(pl_arr[:,0]-xi+100,pl_arr[:,1]-yi+100,'y-',linewidth =3)
        #plt.savefig(name,dpi=fig.dpi )
        self.SaveFigureAsImage(name,fig = fig,orig_size=(np.array(tmpim).shape[0],np.array(tmpim).shape[1]))
        plt.close(fig)
        return name
        
    def ExtractandUpload(self):
...
if __name__ == '__main__':
    testhis = HisImage(r'D:/DATASET/CLRM/',r'2016-01-13 16.44.33_1_Carlos')
    testhis.ExtractandUpload()
