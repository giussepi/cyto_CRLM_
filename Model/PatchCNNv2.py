...
class PatchCNNv2(nn.Module):
    def __init__(self,layers= [2,2,2,2],num_classes=11,base_num = 64,dilation =1 , dropout_rate=0, **kwargs):
        super(PatchCNNv2,self).__init__()
        self.base = ResNet(BasicBlock, layers, **kwargs)
        self.down = nn.Sequential(nn.Conv2d(self.base.outdim,base_num*4,kernel_size=3,stride=2,padding=2,dilation = dilation),\
                                              nn.InstanceNorm2d(base_num*4),
                                              nn.LeakyReLU(0.2, True),
                                              nn.Dropout(p=dropout_rate),
                                              
                                              nn.Conv2d(base_num*4,base_num,kernel_size=3,stride=2,padding=2,dilation = dilation),\
